...
/*
https://app.codility.com/programmers/lessons/4-counting_elements/frog_river_one/v
...
solution(5, new [] { 1, 3, 1, 4, 2, 3, 5, 4 }).Dump();

int solution(int jump, int[] leaves)
{
    var positionsCovered = new HashSet<int>(); // To track covered positions

    for (var time = 0; time < leaves.Length; time++)
    {
        positionsCovered.Add(leaves[time]); // Add the position where leaf falls

        // Check if we have covered all positions from 1 to X
        if (positionsCovered.Count == jump && positionsCovered.All(p => p >= 1 && p <= jump))
            return time; // Return the earliest time
    }
